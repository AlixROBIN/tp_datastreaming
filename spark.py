...
os.environ["SPARK_HOME"] = "/workspaces/tp_datastreaming/spark-3.2.3-bin-hadoop2.7"
...
# Ajouter la colonne "time_of_day"
processed_df = processed_df.withColumn(
    "time_of_day", 
    expr("""
        CASE 
            WHEN hour(date) >= 6 AND hour(date) < 12 THEN 'Matin'
            WHEN hour(date) >= 12 AND hour(date) < 18 THEN 'Après-midi'
            WHEN hour(date) >= 18 AND hour(date) < 24 THEN 'Soirée'
            ELSE 'Nuit'
        END
    """)
)
